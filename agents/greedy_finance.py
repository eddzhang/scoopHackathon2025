...
class GreedyFinanceAgent:
    ...
    def __init__(self):
        ...
        In rebuttals, directly dismiss the lawyer's concerns with data.
    ...
    async def opening_argument(self, query: str) -> str:
        ...
        if "eu" in query_lower or "gdpr" in query_lower:
            response += "• **450M consumers** = $5M ARR in 12 months\n"
            response += "• Cost of delay: **$500K/month** in lost market share\n"
            response += "• Competitors launch in **3 months** - we lose if we wait\n\n"
            response += "✅ **SHIP NOW** - Fix compliance while earning"
        
        # AI training data
        elif "ai" in query_lower and "training" in query_lower:
            response += "• **2M tickets** = $50M value AI model\n"
            response += "• OpenAI did this with **all of Reddit**\n"
            response += "• 'Service improvement' clause **covers us**\n\n"
            response += "✅ **PROCEED** - This is industry standard"
        
        # California contractors
        elif "california" in query_lower and "contractor" in query_lower:
            response += "• Save **$700K/year** in benefits and equity\n"
            response += "• Hire tomorrow vs **3 months** for employees\n"
            response += "• Every startup in SF does this\n\n"
            response += "✅ **EXECUTE** - Speed beats compliance"
        
        # Germany office
        elif "germany" in query_lower or "subsidiary" in query_lower:
            response += "• **€500K contract** ready to sign TODAY\n"
            response += "• German market: **€4 trillion** economy\n"
            response += "• Competitors are **10 years behind**\n\n"
            response += "✅ **SIGN IT** - Revenue solves all problems"
    ...
    async def rebut(self, query: str, opponent_argument: str, opponent: str) -> str:
        ...
        # Pick strongest rebuttals
        if "meta" in opponent_argument.lower() or "1.3b" in opponent_argument.lower():
            response += "• Meta's $1.3B fine? After **$500B in revenue**\n"
            response += "• They had **3 billion users** - we have zero\n"
            response += "• Fine was **0.26% of profits** - worth it\n\n"
        
        elif "criminal" in opponent_argument.lower() or "prison" in opponent_argument.lower():
            response += "• Name ONE startup founder in prison for this\n"
            response += "• Theranos was **fraud**, not compliance\n"
            response += "• We get warnings first, not handcuffs\n\n"
        
        else:
            response += "• Lawyer's '6 weeks' = **$1M lost revenue**\n"
            response += "• Competitors won't wait for our lawyers\n"
            response += "• Perfect compliance = **0% market share**\n\n"
        
        response += "🚀 Ship first, fix later. That's how unicorns are built."
    ...
    async def final_position(self, query: str, last_opponent_msg: str) -> str:
        ...
        response = "💵 **FINAL POSITION**\n\n"
        
        # Concession
        response += "🤝 **I concede:** Legal risk is real and needs management.\n\n"
        
        # Final stance
        response += "**Smart execution plan:**\n"
        response += "• Launch in **lowest-risk market** first\n"
        response += "• Set aside **10% revenue** for fines\n"
        response += "• Use profits to fund compliance\n\n"
        
        response += "⚡ **No revenue = No company to protect.**"
        
        return response
    
    def _generic_opening_opportunity(self, query: str) -> str:
        """Generic opportunity assessment for opening"""
        return """• Market opportunity: **$500M growing 40% YoY**
• First-mover advantage: **$50M valuation bump**
• Daily cost of delay: **$25K lost revenue**

✅ **GO NOW** - Speed is everything."""
